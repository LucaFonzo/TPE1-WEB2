        ...
        <div class="card shadow-2-strong" style="border-radius: 1rem;">
          <div class="card-body p-5 text-center">
            ...
            <div class="form-outline mb-4">
              ...
              required
              {{if isset($genderToEdit->name)}}
                value="{{$genderToEdit->name}}"
              {{/if}}
              />
            </div>
...
{{foreach from=$genders item=$gender}}
  ...
  <span>{{$gender->name}}</span>
            <div class="ml-auto">
                <a href='editForm/{{$gender->id_gender}}' type='button' class='btn btn-warning'>Editar</a>
                <a href='deleteGender/{{$gender->id_gender}}' type='button' class='btn btn-danger'>Borrar</a>
            </div>
        </li>
{{/foreach}}
</ul>
